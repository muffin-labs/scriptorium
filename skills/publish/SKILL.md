...
0c-clearance. **Clearance language is scaffold too, and the converter refuses it.** A footnote that
   ...
   **A desk path is scaffold too, and refused the same way** (Eric, 2026-09-16: *"the footnotes
   should never reference files internal to the writing desk"*). A reader cannot open
   `projects/being-good/facts.md`, `notes.md §H` or `assets/figures.py`, and the path rots when the
   desk moves — a live footnote went stale that way the same day the rule was written. Cite what a
   reader can follow (a public URL — the framework's own docs are public) or say it in words
   (*generated by code, from a fixed seed*), and keep provenance in `publish.yaml`. Link targets
   and image alt text are not judged, so a URL to the same document and an alt transcribing a
   filename shown in a picture both pass.
